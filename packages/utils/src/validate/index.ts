...
const BANK_CARD_MIN_LENGTH = 13
const BANK_CARD_MAX_LENGTH = 19
...
export function validateStrongPassword(value: string): boolean {
  ...
  const hasNumber = /\d/.test(trimmedValue)
  const hasSpecialChar = SPECIAL_CHAR_REGEX.test(trimmedValue)
...
export function validateEmail(value: string): boolean {
  ...
  return emailRegex.test(trimmedValue) && trimmedValue.length <= EMAIL_MAX_LENGTH
}
...
export function validateBankCard(value: string): boolean {
  ...
  const bankCardRegex = new RegExp(`^\\d{${BANK_CARD_MIN_LENGTH},${BANK_CARD_MAX_LENGTH}}$`)
  if (!bankCardRegex.test(trimmedValue)) {
    return false
  }
  // Luhn算法验证
  let sum = 0
